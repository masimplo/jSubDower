...
public class OpenSubtitlesHasher {
	...
	public static String computeHash(File file) throws IOException {
		...
		try {
			long head = computeHashForChunk(fileChannel.map(MapMode.READ_ONLY,
					0, chunkSizeForFile));
	...
	public static String computeHash(InputStream stream, long length)
		...
		byte[] chunkBytes = new byte[(int) Math
				.min(2 * HASH_CHUNK_SIZE, length)];
		...
		// second chunk, or the rest of the data if length is smaller than two
		// chunks
